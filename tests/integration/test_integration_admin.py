...
async def test_admin_get_all_bookings(
    async_client: AsyncClient, admin_token: str, create_booking
):
...
@pytest.mark.asyncio
async def test_admin_without_token(async_client: AsyncClient):
    """Эндпоинт с префиксом /admin недоступен без токена."""
    response = await async_client.get("/admin/bookings")
    assert response.status_code == status.HTTP_401_UNAUTHORIZED
    assert response.json()["detail"] == "Недействительный токен"


@pytest.mark.asyncio
async def test_admin_hide_past(
    ...
    """Параметр hide_past скрывает прошедшие брони."""
    await create_past_booking()
    await create_booking()
    response = await async_client.get(
        "/admin/bookings?hide_past=true",
        headers={"Authorization": f"Bearer {admin_token}"}
    )
    data = response.json()
    assert len(data) == 1
    assert data[0]["booking_date"] == booking_date
