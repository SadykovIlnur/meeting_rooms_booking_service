...
from app.constants import COUNT_ROOMS_INT_TEST
from tests.integration.conftest import post_booking


@pytest.mark.asyncio
async def test_get_rooms(async_client: AsyncClient, employee_token: str):
    ...
    assert len(data) == COUNT_ROOMS_INT_TEST
    for room in data:
        assert "id" in room
        assert "name" in room
...
@pytest.mark.asyncio
async def test_get_rooms_without_token(async_client: AsyncClient):
...
async def test_get_availability_with_booking(
    ...
    await post_booking(async_client, employee_token, room_id=1, timeslot_id=1)
    response = await async_client.get(
        f"/rooms/availability?date_str={booking_date}",
        headers={"Authorization": f"Bearer {employee_token}"}
    )
